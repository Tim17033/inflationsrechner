...
if st.button("📊 Berechnung starten"):
    with st.spinner("Berechnungen werden durchgeführt..."):
        ...
        with col3:
            st.metric(label=f"📈 Betrag mit {zinsrate:.2f}% Zinsen", value=f"{endbetrag_mit_zins:,.2f} €", delta=f"{endbetrag_mit_zins - startbetrag:,.2f} €", delta_color="inverse" if endbetrag_mit_zins < startbetrag else "off")
